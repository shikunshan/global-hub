...
- **主选方案**：通过 Google Favicon API 动态获取高清 favicon
  - URL 格式：`https://www.google.com/s2/favicons?sz=64&domain={hostname}`
  - 从 `site.url` 自动提取域名，无需在数据中手动维护图标
- **Fallback 方案**：Google 加载失败时自动切换到 DuckDuckGo 图标服务
  - URL 格式：`https://icons.duckduckgo.com/ip3/{hostname}.ico`
  - 通过 `<img>` 标签的 `onerror` 事件实现无缝降级
- **数据层**：`sites.json` 中不再需要 `icon` 字段，Logo 完全由 URL 自动推导
- **渲染函数**：`renderLogo(site, sizeClass)` 统一生成带 fallback 的 `<img>` 标签
